...
let upComing_events = []  
let fecha_actual = new Date(datos_constantes.fecha_actual)

 for (const evento of datos_constantes.eventos) {
    let event_date = new Date(evento.fecha)

    
    if (event_date >= fecha_actual) {
        upComing_events.push(evento)
    }
...
for (let i = 0; i < upComing_events.length; i++) {
    let tarjeta = document.createElement("div")
        ...
        tarjeta.innerHTML = `
                    <img src="${upComing_events[i].imagen}" class="card-img-top h-40 object-fit-cover" alt="${upComing_events[i].Categoría}">

                    <div class="card-body d-flex flex-column">
                      <h5 class="card-title "> ${upComing_events[i].nombre}</h5>
                      <p class="card-text flex-grow-1"> ${upComing_events[i].descripción}</p>
                      <p class="card-text">Date: ${upComing_events[i].fecha}</p>
